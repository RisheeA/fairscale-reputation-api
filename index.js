...
// =============================================================================
// x402 PAYMENT MIDDLEWARE (optional — enabled when @x402/express is installed)
// =============================================================================
// Only applied to programmatic API endpoints (/v1/score, /v1/trust-gate, /v1/batch).
// The directory frontend, dashboard, stats, and legacy /score endpoint are ALWAYS FREE.
// Uses the official Coinbase x402 facilitator for payment verification.

let x402Enabled = false;
try {
const { paymentMiddleware } = await import(‘x402-express’);
const PAYMENT_WALLET = process.env.FAIRSCALE_PAYMENT_WALLET || ‘fairAUEuR1SCcHL254Vb3F3XpUWLruJ2a11f6QfANEN’;
...
// Apply x402 ONLY to paid programmatic endpoints
app.use(paymentMiddleware(
PAYMENT_WALLET,
{
‘GET /v1/score’: { price: ‘$0.005’, network: X402_NETWORK },
‘GET /v1/trust-gate’: { price: ‘$0.002’, network: X402_NETWORK },
‘POST /v1/batch’: { price: ‘$0.01’, network: X402_NETWORK },
},
{ url: FACILITATOR_URL }
));
x402Enabled = true;
console.log(’[x402] Payment middleware active. Paid endpoints: /v1/score, /v1/trust-gate, /v1/batch’);
} catch (e) {
// x402-express not installed — all endpoints free (development mode)
console.log(’[x402] x402-express not installed. All endpoints free.’);
}
...
for (const [mint, agent] of REGISTRY.satiAgents) {
  ...
  try {
    ...
    if (feedbacks.length > 0) {
      // Deduplicate by clientAddress — only keep highest-scored feedback per unique reviewer
      const uniqueReviewers = new Map();
      for (const fb of feedbacks) {
        const addr = fb.clientAddress || 'unknown';
        const existing = uniqueReviewers.get(addr);
        if (!existing || (fb.value || 0) > (existing.value || 0)) {
          uniqueReviewers.set(addr, fb);
        }
      }
      const dedupedFeedbacks = Array.from(uniqueReviewers.values());

      const attesters = dedupedFeedbacks.map(fb => ({
        wallet: fb.clientAddress || 'sati_reviewer',
        score: fb.value != null ? fb.value : null, // Don't default to 50 — null means unscored
...
if (feedbackCount > 0 || saidRepScore > 0) {
  // SAID feedback is aggregated — we don't have individual attester wallets.
  // Store the count for display but mark as synthetic (won't count toward boost).
  const attesters = [];
  if (feedbackCount > 0) {
    attesters.push({
      wallet: `said_aggregate`,
      score: saidRepScore > 0 ? Math.min(saidRepScore * 10, 80) : null,
      timestamp: null, type: 'said_aggregate', count: feedbackCount,
    });
  }
...
```
  ...
  if (attesters.length > 0) {
    // Deduplicate attesters by wallet
    const uniqueMap = new Map();
    for (const a of attesters) {
      if (!uniqueMap.has(a.wallet) || (a.score || 0) > (uniqueMap.get(a.wallet).score || 0)) {
        uniqueMap.set(a.wallet, a);
      }
    }
    const dedupedAttesters = Array.from(uniqueMap.values());

    const existing = REGISTRY.attestationGraph.get(wallet);
    // Merge with existing, deduplicating again
    const allAttesters = existing ? [...existing.attesters, ...dedupedAttesters] : dedupedAttesters;
    const mergedMap = new Map();
    for (const a of allAttesters) {
      if (!mergedMap.has(a.wallet) || (a.score || 0) > (mergedMap.get(a.wallet).score || 0)) {
        mergedMap.set(a.wallet, a);
      }
    }
...
// Attestation graph boost (up to +12 points, heavily quality-gated)
// Anti-gaming: only unique attesters count, attesters must have real scores,
// and low-quality attesters are worth very little.
const graphData = REGISTRY.attestationGraph.get(wallet);
let attestationBoost = 0;
if (graphData && graphData.attesters) {
// Deduplicate by wallet address — only count each unique attester once
const uniqueAttesters = new Map();
for (const a of graphData.attesters) {
if (!a.wallet || a.wallet.startsWith(‘said_feedback_’)) continue; // Skip synthetic attesters
if (!uniqueAttesters.has(a.wallet) || (a.score || 0) > (uniqueAttesters.get(a.wallet).score || 0)) {
uniqueAttesters.set(a.wallet, a);
}
}

```
const unique = Array.from(uniqueAttesters.values());
const uniqueCount = unique.length;

// Only count attesters with a real score above a quality threshold
const highQuality = unique.filter(a => a.score != null && a.score >= 40); // Score 40+ required
const veryHighQuality = unique.filter(a => a.score != null && a.score >= 60); // Score 60+ is premium

// Count boost: modest, diminishing returns, only from quality attesters
// 1pt per quality attester (40+), capped at 4
const countBoost = Math.min(highQuality.length, 4);

// Quality boost: average score of high-quality attesters
const avgHighScore = highQuality.length > 0
  ? highQuality.reduce((s, a) => s + a.score, 0) / highQuality.length
  : 0;
// Only meaningful if avg is high (60+ gives 2pts, 80+ gives 4pts)
const qualityBoost = avgHighScore >= 80 ? 4 : avgHighScore >= 60 ? 2 : avgHighScore >= 40 ? 1 : 0;

// Premium attester bonus: having a very high-scoring attester (60+) vouch for you
const premiumBoost = veryHighQuality.length >= 3 ? 4 : veryHighQuality.length >= 1 ? 2 : 0;

attestationBoost = Math.min(countBoost + qualityBoost + premiumBoost, 12);

// Update the graph data with deduplicated count for display
graphData.unique_attester_count = uniqueCount;
graphData.quality_attester_count = highQuality.length;
```
...
x402: x402Enabled ? ‘active’ : ‘disabled’,
});
});

// — x402 Pricing Info (always free) —
app.get(’/v1/pricing’, (req, res) => {
res.json({
x402_enabled: x402Enabled,
protocol: ‘x402’,
facilitator: ‘https://x402.org/facilitator’,
network: process.env.X402_NETWORK || ‘solana-mainnet’,
payment_wallet: ‘fairAUEuR1SCcHL254Vb3F3XpUWLruJ2a11f6QfANEN’,
paid_endpoints: {
‘GET /v1/score’: ‘$0.005’,
‘GET /v1/trust-gate’: ‘$0.002’,
‘POST /v1/batch’: ‘$0.01’,
},
free_endpoints: [
‘GET /directory’,
...
note: x402Enabled
? ‘Paid endpoints require x402 payment via the facilitator. Free endpoints have no payment requirement.’
: ‘x402 is not currently active. All endpoints are free.’,
});
});
